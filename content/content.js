...
const DEFAULT_VALUES_JSON_PATH = chrome.runtime.getURL(`${BASE_PATH}/defaultValues.json`);
const STORAGE_KEYS_JSON_PATH = chrome.runtime.getURL(`${BASE_PATH}/storageKeys.json`);
	...
	if (!event.shiftKey) {
		...
		floatingPDFTimeoutID = window.setTimeout(() => {
			...
			if (targetHref.match(REGEX_PDF) === null) {
				...
				src = targetHref.match(REGEX_PDF)[0];
			} else {
			...
			if (src !== '') {
				...
				FLOATING_PDF.setAttribute('src', src);
...
const VIDEO_EXTENSIONS = ['3gp', 'm3u8', 'mkv', 'mov', 'mp4', 'webm', 'wmv'];

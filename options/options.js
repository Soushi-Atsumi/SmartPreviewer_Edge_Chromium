...
async function main() {
	...
	INITIALIZE_OPTIONS_BUTTON.innerText = chrome.i18n.getMessage('initializeOptions');
...
function refreshingInputOnClick(event) {
	switch (event.target.id) {
		case REFRESHING_ENABLED_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.refreshing.enabled]: REFRESHING_ENABLED_INPUT.checked });
			break;
		case REFRESHING_INTERVAL_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.refreshing.interval]: parseFloat(REFRESHING_INTERVAL_INPUT.value) });
			REFRESHING_INTERVAL_VALUE_LABEL.innerText = REFRESHING_INTERVAL_INPUT.value;
			break;
	}
}
...
async function checkRefreshing(_) {
	const OPTIONS = await chrome.storage.sync.get(Object.values(storageKeys.refreshing));
...
function audioInputOnClick(event) {
	switch (event.target.id) {
		case AUDIO_ASPECT_RATIO_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.audio.aspectRatio]: parseFloat(AUDIO_ASPECT_RATIO_INPUT.value) });
			AUDIO_ASPECT_RATIO_VALUE_LABEL.innerText = AUDIO_ASPECT_RATIO_INPUT.value;
			break;
		case AUDIO_AUTOPLAY_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.audio.autoplay]: AUDIO_AUTOPLAY_INPUT.checked });
			break;
		case AUDIO_DELAY_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.audio.delay]: parseFloat(AUDIO_DELAY_INPUT.value) });
			AUDIO_DELAY_VALUE_LABEL.innerText = AUDIO_DELAY_INPUT.value;
			break;
		case AUDIO_ENABLED_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.audio.enabled]: AUDIO_ENABLED_INPUT.checked });
			break;
		case AUDIO_LOOP_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.audio.loop]: AUDIO_LOOP_INPUT.checked });
			break;
		case AUDIO_VOLUME_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.audio.volume]: parseFloat(AUDIO_VOLUME_INPUT.value) });
			AUDIO_VOLUME_VALUE_LABEL.innerText = AUDIO_VOLUME_INPUT.value;
			break;
	}
}
...
async function checkAudio() {
	const OPTIONS = await chrome.storage.sync.get(Object.values(storageKeys.audio));
	const KEYS = Object.keys(OPTIONS);
	for (let i = 0; i < KEYS.length; i++) {
		switch (KEYS[i]) {
...
function imageInputOnClick(event) {
	switch (event.target.id) {
		case IMAGE_ASPECT_RATIO_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.image.aspectRatio]: IMAGE_ASPECT_RATIO_INPUT.value });
			IMAGE_ASPECT_RATIO_VALUE_LABEL.innerText = IMAGE_ASPECT_RATIO_INPUT.value;
			break;
		case IMAGE_BORDER_WIDTH_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.image.borderWidth]: IMAGE_BORDER_WIDTH_INPUT.value });
			IMAGE_BORDER_WIDTH_VALUE_LABEL.innerText = IMAGE_BORDER_WIDTH_INPUT.value;
			break;
		case IMAGE_DELAY_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.image.delay]: parseFloat(IMAGE_DELAY_INPUT.value) });
			IMAGE_DELAY_VALUE_LABEL.innerText = IMAGE_DELAY_INPUT.value;
			break;
		case IMAGE_ENABLED_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.image.enabled]: IMAGE_ENABLED_INPUT.checked });
			break;
	}
}

async function checkImage() {
	const OPTIONS = await chrome.storage.sync.get(Object.values(storageKeys.image));
	const KEYS = Object.keys(OPTIONS);
	for (let i = 0; i < KEYS.length; i++) {
		switch (KEYS[i]) {
...
function pdfInputOnClick(event) {
	switch (event.target.id) {
		case PDF_ASPECT_RATIO_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.pdf.aspectRatio]: PDF_ASPECT_RATIO_INPUT.value });
			PDF_ASPECT_RATIO_VALUE_LABEL.innerText = PDF_ASPECT_RATIO_INPUT.value;
			break;
		case PDF_BORDER_WIDTH_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.pdf.borderWidth]: PDF_BORDER_WIDTH_INPUT.value });
			PDF_BORDER_WIDTH_VALUE_LABEL.innerText = PDF_BORDER_WIDTH_INPUT.value;
			break;
		case PDF_DELAY_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.pdf.delay]: parseFloat(PDF_DELAY_INPUT.value) });
			PDF_DELAY_VALUE_LABEL.innerText = PDF_DELAY_INPUT.value;
			break;
		case PDF_ENABLED_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.pdf.enabled]: PDF_ENABLED_INPUT.checked });
			break;
	}
}

async function checkPDF() {
	const OPTIONS = await chrome.storage.sync.get(Object.values(storageKeys.pdf));
	const KEYS = Object.keys(OPTIONS);
	for (let i = 0; i < KEYS.length; i++) {
		switch (KEYS[i]) {
...
function videoInputOnClick(event) {
	switch (event.target.id) {
		case VIDEO_ASPECT_RATIO_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.video.aspectRatio]: parseFloat(VIDEO_ASPECT_RATIO_INPUT.value) });
			VIDEO_ASPECT_RATIO_VALUE_LABEL.innerText = VIDEO_ASPECT_RATIO_INPUT.value;
			break;
		case VIDEO_AUTOPLAY_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.video.autoplay]: VIDEO_AUTOPLAY_INPUT.checked });
			break;
		case VIDEO_DELAY_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.video.delay]: parseFloat(VIDEO_DELAY_INPUT.value) });
			VIDEO_DELAY_VALUE_LABEL.innerText = VIDEO_DELAY_INPUT.value;
			break;
		case VIDEO_ENABLED_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.video.enabled]: VIDEO_ENABLED_INPUT.checked });
			break;
		case VIDEO_LOOP_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.video.loop]: VIDEO_LOOP_INPUT.checked });
			break;
		case VIDEO_VOLUME_INPUT.id:
			chrome.storage.sync.set({ [storageKeys.video.volume]: parseFloat(VIDEO_VOLUME_INPUT.value) });
			VIDEO_VOLUME_VALUE_LABEL.innerText = VIDEO_VOLUME_INPUT.value;
			break;
	}
}
...
async function checkVideo() {
	const OPTIONS = await chrome.storage.sync.get(Object.values(storageKeys.video));
	const KEYS = Object.keys(OPTIONS);
	for (let i = 0; i < KEYS.length; i++) {
		switch (KEYS[i]) {
